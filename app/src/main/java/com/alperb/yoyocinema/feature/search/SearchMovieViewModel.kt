...
class SearchMovieViewModel @Inject constructor(
    ...
    //val selectedSortingOption: MutableLiveData<MovieSortModel> = MutableLiveData(MovieSortModel())

    var selectedSortingOption = MovieSortModel()

    val movieListState: LiveData<UIState<List<YoyoMovieOverview>>> = Transformations.switchMap(queriedMovie) { query ->
        liveData {
            emit(UIState.Loading)
            emit(searchMovieUseCase.searchMovie(query.orEmpty(), selectedSortingOption))
        }
    }
